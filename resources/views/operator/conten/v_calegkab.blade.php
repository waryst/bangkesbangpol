                    ...
                    <div class="text-center mx-auto col-12 my-2 ">
                        @if ($pilih_tps->id ?? 0 != null)
                            ...
                            <fieldset class="scheduler-border col-md-2 pb-2 px-0">
                                ...
                                <span class="loadingsuara" id="save{{ $pilih_tps->id }}"
                            ...
                            <div class="card card_suara">
                                ...
                                <ul class="list-group list-group-flush">
                                    <li class="list-group-item p-0 my-0 d-flex justify-content-start ">
                                        <div class="d-flex align-items-center nomer_suara col-1">
                                            {{ sprintf('%02d', $partai->no_urut) }}
                                        </div>
                                        <div class="d-flex align-items-center nama_partai col-8" style="height: 40px">
                                            {{ $partai->nama }}
                                        </div>
                                        @if ($pilih_tps->id ?? 0 != null)
                                            <div class="d-flex  align-items-center justify-content-center col-3">
                                                <input type="number" min="0" id="{{ $partai->id }}"
                                                    data-default="{{ $partai->suarapartaikab[0]->jumlah ?? 0 }}"
                                                    onKeyPress="if(this.value.length==3) return false;"
                                                    oninput="this.value = Math.abs(this.value)" autocomplete="off"
                                                    value="{{ $partai->suarapartaikab[0]->jumlah ?? 0 }}"
                                                    class="form-control text-center submitpartai p-0 mr-1 font-weight-bold"
                                                    style="height: 20px;font-size: 13px;">
                                                <span class="loadingsuara" id="save{{ $partai->id }}">
                                                </span>
                                            </div>
                                        @endif
                                    </li>
                                            ...
                                            @if ($pilih_tps->id ?? 0 != null)
                                                <div class="d-flex align-items-center float-right col-3">
                                                    <input type="number" min="0" id="{{ $caleg->id }}"
                                                        data-default="{{ $caleg->suaracalegkab[0]->jumlah ?? 0 }}"
                                                        data-partai="{{ $partai->id }}" onfocus="this.placeholder = ''"
                                                        onKeyPress="if(this.value.length==3) return false;"
                                                        oninput="this.value = Math.abs(this.value)" autocomplete="off"
                                                        value="{{ $caleg->suaracalegkab[0]->jumlah ?? 0 }}"
        ...
        $(document).ready(function() {
            ...
            $('.submittidaksah').blur(function() {
                ...
                if (jumlah_suara != $("#" + input_id).attr("data-default")) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{ url('savesuara/suaratidaksahdpr_kab') }}",
                        data: fd,
                        contentType: false,
                        processData: false,
                        dataType: 'json',
            ...
            $('.submitpartai').on('keyup click', function(e) {
                if (e.keyCode != 9) {
                    input_id = $(this).attr('id');
                    ...
                    if (jumlah_suara != $("#" + input_id).attr("data-default")) {
                        $('#save' + input_id).html(`   `);
                        clearTimeout(Interval);
                        Interval = setTimeout(saveSuaraPartai, 500);
                    }
                }
            });
            $('.submitpartai').blur(function() {
                input_id = $(this).attr('id');
                tps_id = "{{ $pilih_tps->id ?? 0 }}";
                jumlah_suara = $(this).val();
                if (jumlah_suara != $("#" + input_id).attr("data-default")) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            $('#save' + data.partai_id).html(`
                                <svg viewBox="0 0 110 110" width="10" height="10">
                                <path d="M0 64.37a9.67 9.67 0 0 1 2.94-4.67 8 8 0 0 1 9.2-.66 57.21 57.21 0 0 1 13.8 11 114.1 114.1 0 0 1 13.18 16.73c.17.26.36.5.56.77 1.83-3.43 3.54-6.85 5.44-10.17C56 58.23 70 41.83 88.16 29.21a125.64 125.64 0 0 1 28.44-14.62c5.76-2.12 11.08 1.82 11.22 6.91a1.32 1.32 0 0 0 .18.43v.24c-.11.49-.2 1-.32 1.47a7.91 7.91 0 0 1-5.35 5.95 105 105 0 0 0-25.56 13.15 125.27 125.27 0 0 0-33.1 34.91A138 138 0 0 0 48.5 108.5a7.69 7.69 0 0 1-6.15 5.27 4.66 4.66 0 0 0-.64.23h-1.44c-.1-.06-.19-.16-.3-.18a8.17 8.17 0 0 1-6.42-4.82 128.9 128.9 0 0 0-15.12-23.32c-3.76-4.53-7.75-8.87-12.87-11.88C2.92 72.25.87 70.46 0 67.48z" fill="#1148f1"></path>
                                </svg>          
                            `);
                            document.getElementById(data.partai_id).setAttribute("data-default",
                                data.jumlah_suara);
                        },
                        error: function(data) {
                            var errors = data.responseJSON;
            ...
            $('#tps').change(function() {
                var tps_id = $('#tps').val();
                window.location = "{{ url('calegkab') }}/" + tps_id;
            });
        });
        ...
        function saveSuaraPartai() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    `);
                    document.getElementById(data.partai_id).setAttribute("data-default", data.jumlah_suara);
                },
                error: function(data) {
                    var errors = data.responseJSON;
                    if ($.isEmptyObject(errors) == false) {
