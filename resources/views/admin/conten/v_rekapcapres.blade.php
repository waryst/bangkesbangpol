                            ...
                            <table id="example1" class="table table-head-fixed table-bordered table-hover text-wrap">
                                <thead>
                                    <tr>
                                        ...
                                        @foreach ($data_capres as $capres)
                                            <th class=" text-center">0{{ $capres->no_urut }}<p>
                                                    {{ $capres->nama_capres }}<br>{{ $capres->nama_cawapres }}</th>
                                        @endforeach
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($data as $data)
                                        <tr>
                                            <td>
                                                @if ($ket == 'kab')
                                                    <a href="{{ url('rekapcapres/kec/' . $data->id) }}">
                                                        {{ $data->title }}
                                                    </a>
                                                @elseif($ket == 'kec')
                                                    <a href="{{ url('rekapcapres/desa/' . $data->id) }}">
                                                        {{ $data->title }}
                                                    </a>
                                                @else
                                                    TPS {{ $data->title }}
                                            ...
                                            @foreach ($data_capres as $detail_capres)
                                                @if ($data->suaracapres->where('capres_id', $detail_capres->id)->groupBy('capres_id')->count() === 0)
                                                    <td class="text-center"> 0</td>
                                                @else
                                                    @foreach ($data->suaracapres->where('capres_id', $detail_capres->id)->groupBy('capres_id') as $hasil)
...
@push('java')
    ...
    <script type="text/javascript">
        var data = {
            ...
            labels: [
                @foreach ($data_capres as $label_capres)
                    '{{ $label_capres->nama_capres }} - {{ $label_capres->nama_cawapres }}',
                @endforeach
            ]
        };
