            ...
            <fieldset class="scheduler-border">
                ...
                <div class="card">
                    ...
                    <p class="mx-auto font-italic font-weight-normal" style="position: absolute;left: 1%;top:95%">
                        Total Suara Masuk : <span class="font-weight-bold">{{ $suarasah + $suaratidaksah }}</span> |
                        Suara Sah
                        :
                            ...
                            <table id="example1" class="table table-head-fixed table-bordered table-hover text-wrap">
                                <thead>
                                    <tr>
                                        ...
                                        @foreach ($data_cagub as $cagub)
                                            <th class=" text-center">0{{ $cagub->no_urut }}<p>
                                                    {{ $cagub->nama_cagub }}<br>{{ $cagub->nama_cawagub }}</th>
                                        @endforeach
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($data as $data)
                                        <tr>
                                            <td>
                                                @if ($ket == 'kab')
                                                    <a href="{{ url('rekapcagub/kec/' . $data->id) }}">
                                                        {{ $data->title }}
                                                    </a>
                                                @elseif($ket == 'kec')
                                                    <a href="{{ url('rekapcagub/desa/' . $data->id) }}">
                                                        {{ $data->title }}
                                                    </a>
                                                @else
                                                    TPS {{ $data->title }}
                                            ...
                                            @foreach ($data_cagub as $detail_cagub)
                                                @if ($data->suaracagub->where('cagub_id', $detail_cagub->id)->groupBy('cagub_id')->count() === 0)
                                                    <td class="text-center"> 0</td>
                                                @else
                                                    @foreach ($data->suaracagub->where('cagub_id', $detail_cagub->id)->groupBy('cagub_id') as $hasil)
                                ...
                                <tfoot>
                                    <tr style="background-color: #dfdcdc">
                                        ...
                                        @foreach ($data_cagub as $suara_cagub)
                                            <th rowspan="1" colspan="1" class="text-center">
                                                {{ $suara_cagub->suaracagub_sum_jumlah }}</th>
                                        @endforeach
                                    </tr>
                                </tfoot>
                                </tbody>
    ...
    <script type="text/javascript">
        var data = {
            datasets: [{
                data: [
                    @foreach ($data_cagub as $suara_cagub)
                        "{{ $suara_cagub->suaracagub_sum_jumlah }}",
                    @endforeach
                ],
                backgroundColor: [
                    "#FF6384",
            ...
            labels: [
                @foreach ($data_cagub as $label_cagub)
                    '{{ $label_cagub->nama_cagub }} - {{ $label_cagub->nama_cawagub }}',
                @endforeach
            ]
        };
