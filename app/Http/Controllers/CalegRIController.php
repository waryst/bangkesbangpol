...
class CalegRIController extends Controller
{
    /**
    ...
    public function index()
    {
        $data = Partai::withCount([
            'caleg as jumlah_caleg' => function ($query) {
                $query->where('dapil_id', 1);
            }])->orderBy('no_urut', 'ASC')->get();
        return view('admin.content.calegri', compact('data'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // $nama = Caleg::where('partai_id', $request->partai_id)->where('dapil_id', 1)->where('nama', $request->nama)->count();
        $urut = Caleg::where('partai_id', $request->partai_id)->where('dapil_id', 1)->where('no_urut', $request->no_urut)->count();
        ...
        return response()->json([
            ...
            'data'    => Caleg::where('partai_id', $request->partai_id)->where('dapil_id', 1)->orderBy('no_urut', 'ASC')->get()
        ]);
    }
    ...
    {
        $data = Caleg::where('partai_id', $id)->where('dapil_id', 1)->orderBy('no_urut', 'ASC')->get();
        return response()->json([
            'status' => true,
            'data'=> $data
        ]);
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($caleg) {
            if ($field=='no_urut') {
                $other = Caleg::where('partai_id', $caleg->partai_id)->where('dapil_id', 1)->where('no_urut', $value);
                if ($other->count()!=0) {
                    $other->first()->update(['no_urut' => null]);
                }
                $caleg->update([$field => $value]);
                $data =  Caleg::where('partai_id', $caleg->partai_id)->where('dapil_id', 1)->orderBy('no_urut', 'ASC')->get();
                $status = "success";
                $type = "success";
